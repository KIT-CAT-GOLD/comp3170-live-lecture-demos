...
public class RenderTextureQuad extends SceneObject {
	...
	public void drawSelf(Matrix4f mvpMatrix) {
		...
		// screen size
		shader.setUniform("u_screenSize", size);
		shader.setUniform("u_focusDepth", focusDepth);
	...
	private static final float FOCUS_SPEED = 0.2f;
	
	public void update(float deltaTime, InputManager input) {

		if (input.isKeyDown(GLFW_KEY_KP_ADD)) {
			focusDepth += FOCUS_SPEED * deltaTime;			
			focusDepth = Math.min(focusDepth, 1f);
		}
		if (input.isKeyDown(GLFW_KEY_KP_SUBTRACT)) {
			focusDepth -= FOCUS_SPEED * deltaTime;			
			focusDepth = Math.max(focusDepth, 0f);
		}
	}
